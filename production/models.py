...
class SalesTimestamp(models.Model):
    sale = models.ForeignKey(ReadyForSaleStock, on_delete=models.SET_NULL, null=True)
    quantity_sold = models.IntegerField(default=0, null=True, blank=True)
    amount_sold = models.IntegerField(default=0, null=True, blank=True)
    sale_branch = models.CharField(max_length=400,null=True,blank=True)
    date_sold = models.DateField(null=True, blank=True)  
...
class RasiseProductionNotification(models.Model):
    stock = models.ForeignKey(ReadyForSaleStock, on_delete=models.SET_NULL, null=True) 
